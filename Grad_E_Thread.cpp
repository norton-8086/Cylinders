...
__fastcall TGrad_E_Thread::TGrad_E_Thread(bool CreateSuspended)
	: TThread(CreateSuspended)
{
}

void __fastcall TGrad_E_Thread::New_Form(){
...
void __fastcall TGrad_E_Thread::Change_param(){
...
void __fastcall TGrad_E_Thread::Plot(){
...
void E_der(struct pair_of_cyl *pair){
	int n1, n2;
	n1 = pair->cyl1.n;
	n2 = pair->cyl2.n;
	long double der_E = (long double) 0.0;

	for(int i = 0; i < n1; i++){
		der_E = (long double) 0.0;

		for(int j = 0; j < i; j++){
			der_E += pair->arr_E_z.array[j][i] * pair->cyl1.rings[j].E_z;
		}
		...
		for(int j = n1; j < n1 + n2; j++){
			der_E += pair->arr_E_z.array[j][i] * pair->cyl2.rings[j - n1].E_z;
		}

		pair->L_E.data[i] = (long double)2.0*der_E;
	}
...
void E_shift(struct pair_of_cyl *pair, long double dt, long double betta){
	int n1, n2;
	n1 = pair->cyl1.n;
	n2 = pair->cyl2.n;

	for(int i = 0; i < n1; i++){
		pair->v_E.data[i] = betta*(pair->v_E.data[i] - pair->L_E.data[i] * dt);
		pair->cyl1.rings[i].Q += pair->v_E.data[i]*dt;
	}
	for(int i = n1; i < n1 + n2; i++){
		pair->v_E.data[i] = betta*(pair->v_E.data[i] - pair->L_E.data[i] * dt);
		pair->cyl2.rings[i - n1].Q += pair->v_E.data[i]*dt;
	}
...
void E_grad_step(struct pair_of_cyl *pair, long double *dt, long double *betta, long double dt_change){
	...
	if(  pair->W < W_prev && pair->dev_fi < dev_fi_prev &&  pair->dev_E < dev_E_prev){
		...
		for(int i = 0; i < n1; i++){
			pair->cyl1.rings[i].Q -= pair->v_E.data[i] **dt;
			pair->der_E_v_colomn.data[i] = pair->v_E.data[i] / *betta + pair->L_E.data[i] **dt;
		}
...
//---------------------------------------------------------------------------
void __fastcall TGrad_E_Thread::Execute()
{
	...
	for(int i = 0; i < n1 + n2; i++){
		pair.der_E_colomn.data[i] = pair.der_E_v_colomn.data[i] = (long double)0.0;
	}
	for(int i = 0; i < n1 + n2; i++){
		pair.L_E.data[i] = pair.v_E.data[i] = (long double)0.0;
	}
//--------------–¿—◊®“ Õ¿ ”ƒ¿À≈Õ»»----------------------------------------------
	int i = 1;
	dt_now = dt;
	while(dt_now > dt_stop){
		E_grad_step(&pair, &dt_now, &betta, dt_change);
		if(i%frames == 0){
			Synchronize(&Plot);
		}
		i++;
...
//-----------------ƒ¬»∆≈Õ»≈ ¬œ–¿¬Œ----------------------------------------------
	while(z_now <= dz_max){
		...
		dt_now = dt;
		i = 1;
		while(dt_now >= dt_stop && pair.dev_E > ep){
			if(go_flag == false){
				while(go_flag != true){
				}
			}
...
//----------------------«¿œ»—‹ œŒ ‘¿…À¿Ã----------------------------------------
	...
	chapter_name = "Grad_E_R1_" + std::to_string(pair.cyl1.R) + "_R2_" + std::to_string(pair.cyl2.R) + "_L1_" + std::to_string(pair.cyl1.L) + "_L2_" + std::to_string(pair.cyl2.L) + "_Q1_" + std::to_string(pair.cyl1.Q) + "_Q2_" + std::to_string(pair.cyl2.Q) + "_n1_" + std::to_string(pair.cyl1.n) +  "_n2_" + std::to_string(pair.cyl2.n) + "_dt_stop_" + std::to_string(dt_stop) + "_steps_" + std::to_string(steps);
