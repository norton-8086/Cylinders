...
void __fastcall TGrad_Fi_Thread::New_Form(){
...
void __fastcall TGrad_Fi_Thread::Change_param(){
...
void __fastcall TGrad_Fi_Thread::Plot(){
...
__fastcall TGrad_Fi_Thread::TGrad_Fi_Thread(bool CreateSuspended)
	: TThread(CreateSuspended)
{
}

void Fi_der(struct pair_of_cyl *pair){
	int n1, n2;
	n1 = pair->cyl1.n;
	n2 = pair->cyl2.n;
	long double der_Fi = (long double) 0.0;

	for(int i = 0; i < n1; i++){
		der_Fi = (long double) 0.0;

		for(int j = 0; j < i; j++){
			der_Fi += pair->arr_Fi.array[j][i] * (pair->cyl1.rings[j].fi - pair->cyl1.fi);
		}
		...
		for(int j = n1; j < n1 + n2; j++){
			der_Fi += pair->arr_Fi.array[j][i] * (pair->cyl2.rings[j - n1].fi - pair->cyl2.fi);
		}

		pair->L_Fi.data[i] = (long double)2.0*der_Fi;
	}
...
void Fi_shift(struct pair_of_cyl *pair, long double dt, long double betta){
	int n1, n2;
	n1 = pair->cyl1.n;
	n2 = pair->cyl2.n;

	for(int i = 0; i < n1; i++){
		pair->v_Fi.data[i] = betta*(pair->v_Fi.data[i] - pair->L_Fi.data[i] * dt);
		pair->cyl1.rings[i].Q += pair->v_Fi.data[i]*dt;
	}
	for(int i = n1; i < n1 + n2; i++){
		pair->v_Fi.data[i] = betta*(pair->v_Fi.data[i] - pair->L_Fi.data[i] * dt);
		pair->cyl2.rings[i - n1].Q += pair->v_Fi.data[i]*dt;
	}
...
void Fi_grad_step(struct pair_of_cyl *pair, long double *dt, long double *betta, long double dt_change){
	...
	if(pair->W < W_prev && pair->dev_fi < dev_fi_prev && pair->dev_E < dev_E_prev){
		...
		for(int i = 0; i < n1; i++){
			pair->cyl1.rings[i].Q -= pair->v_Fi.data[i] **dt;
			pair->der_Fi_v_colomn.data[i] = pair->v_Fi.data[i] / *betta + pair->L_Fi.data[i] **dt;
		}
...
//---------------------------------------------------------------------------
void __fastcall TGrad_Fi_Thread::Execute()
{
	...
	for(int i = 0; i < n1 + n2; i++){
		pair.der_Fi_colomn.data[i] = pair.der_Fi_v_colomn.data[i] = (long double)0.0;
	}
	for(int i = 0; i < n1 + n2; i++){
		pair.L_Fi.data[i] = pair.v_Fi.data[i] = (long double)0.0;
	}
//--------------–¿—◊®“ Õ¿ ”ƒ¿À≈Õ»»----------------------------------------------
	int i = 1;
	dt_now = dt;
	while(dt_now > dt_stop){
		Fi_grad_step(&pair, &dt_now, &betta, dt_change);
		if(i%frames == 0){
			Synchronize(&Plot);
		}
		i++;
...
//--------------–¿—◊®“ ¬ ÷≈Õ“–≈-------------------------------------------------
	...
	long double F_stop = ep*pair.dev_fi;
    Synchronize(&Plot);
//-----------------ƒ¬»∆≈Õ»≈ ¬œ–¿¬Œ----------------------------------------------
	while(z_now <= dz_max){
		update(&pair);
		...
		i = 1;
		while(dt_now >= dt_stop && pair.dev_fi> F_stop){
			if(go_flag == false){
				while(go_flag != true){
				}
			}
...
//----------------------«¿œ»—‹ œŒ ‘¿…À¿Ã----------------------------------------
	...
	chapter_name = "Grad_Fi_R1_" + std::to_string(pair.cyl1.R) + "_R2_" + std::to_string(pair.cyl2.R) + "_L1_" + std::to_string(pair.cyl1.L) + "_L2_" + std::to_string(pair.cyl2.L) + "_Q1_" + std::to_string(pair.cyl1.Q) + "_Q2_" + std::to_string(pair.cyl2.Q) + "_n1_" + std::to_string(pair.cyl1.n) +  "_n2_" + std::to_string(pair.cyl2.n) + "_dt_stop_" + std::to_string(dt_stop) + "_steps_" + std::to_string(steps);
